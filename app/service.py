...
async def run_monitor(monitor: dict) -> dict:
    ...
    try:
        logs_text = await _collect_monitor_logs(monitor)
        if not logs_text:
            raise RuntimeError("No log input collected")
...
async def _collect_monitor_logs(monitor: dict) -> str:
    ...
    pending: list[tuple[str, asyncio.Future[str]] | tuple[str, asyncio.Future]] = []
    for input_def in inputs:
        ...
        future = loop.run_in_executor(None, _run_command, command)
